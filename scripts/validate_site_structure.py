...
if roll_cards:
    ...
    require(len(entries) >= 53, f'Ship\'s Roll appears truncated: only {len(entries)} ordinary entries found')
else:
...
require('CREW HONORS — DAY 21' in honors, 'Current Crew Honors is not Day 21')
require('day-21-crew-honors.jpg' in honors, 'Current Crew Honors is not using Day 21 artwork')
for token in ('FRED', 'AUÐUN', 'JAGADISH', 'EINAR', 'ANA', 'RAGNA'):
    require(token in honors, f'Current Crew Honors is missing {token}')
require('CREW HONORS — DAY 20' not in honors, 'Day 20 is still present in current Crew Honors')
require('id="honors-day-21"' not in html, 'Duplicate standalone Day 21 Crew Honors section exists')
...
# Voyage Archive continuity through Day 21.
...
require('DAY 21 · 95 VIKINGS · 37,954,948 STEPS · ONE LIVING SAGA' in html,
        'Closing Day 21 voyage summary is missing or stale')
