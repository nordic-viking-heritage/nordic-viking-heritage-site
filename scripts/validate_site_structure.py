...
def section(text,sid):
    start=re.compile(r'<section\b(?=[^>]*\bid=["\']'+re.escape(sid)+r'["\'])[^>]*>',re.I)
    matches=list(start.finditer(text))
    if len(matches)!=1:
        fail(f'Section {sid!r} must occur exactly once; found {len(matches)}'); return ''
    begin=matches[0].start(); nxt=re.search(r'<section\b',text[matches[0].end():],re.I)
    end=matches[0].end()+nxt.start() if nxt else len(text)
...
for sid,label,destination,total in [('chapter-one-complete','CHAPTER I','NORTH ATLANTIC CROSSING','19,155,478'),('chapter-two-complete','CHAPTER II','NEWFOUNDLAND','30,701,056'),('chapter-three-complete','CHAPTER III','NOVA SCOTIA','50,647,198')]:
    block=sections.get(sid,''); upper=block.upper(); require(label in upper,f'{label} identity is missing'); require(destination in upper,f'{label} destination marker is missing'); require(total in block,f'{label} completion total changed')
require('CHAPTER IV AWAITS' in sections.get('chapter-three-complete','').upper(),'Chapter III closing marker changed')
ship=sections.get('ship-roll',''); require('THE CAPTAIN' in ship and 'HÁKON' in ship,"Captain role card is missing from Ship's Roll"); require('KEEPER OF THE SAGA' in ship and 'ÞÓRA' in ship,"Keeper role card is missing from Ship's Roll")
require(len(re.findall(r'<article\b',ship,re.I))>=68,"Ship's Roll appears truncated")
...
for token in ('50,647,198','103','4,329,168','67.53','day-26-voyage-map.jpg','day-26-crew-honors.jpg','CHAPTER III — COMPLETE','ARRIVED — NOVA SCOTIA'): require(token in d26,f'Day 26 archive canonical token missing: {token}')
